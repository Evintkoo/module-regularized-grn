/// Train model with Classification Head (Top Tower)
/// Uses MLP classifier instead of dot product for potentially better accuracy
use module_regularized_grn::{
    Config,
    models::{classifier_head::HybridWithClassifier, nn::bce_loss_backward},
...
fn main() -> Result<()> {
    println!("═══════════════════════════════════════════════════════════════════");
    println!("   TRAINING WITH CLASSIFICATION HEAD (TOP TOWER)");
    println!("═══════════════════════════════════════════════════════════════════\n");
    ...
    let hidden_dim = 2048;
    let output_dim = 512;
    let classifier_hidden = 512;  // Classification head hidden size
    let initial_lr = 0.003;
    let min_lr = 0.0003;
    let batch_size = 256;
    let epochs = 100;
    ...
    println!("  Classifier hidden: {}", classifier_hidden);
    ...
    // Train ensemble
    println!("═══════════════════════════════════════════════════════════════════");
    println!("Training {} models with Classification Head...\n", num_models);
    ...
    println!("🏆 ENSEMBLE PERFORMANCE (WITH CLASSIFICATION HEAD):");
    ...
    println!("Previous best (dot product):  83.06%");
    println!("Current (classifier head):    {:.2}%", ensemble_acc * 100.0);
    println!("Improvement:                  {:+.2}%", (ensemble_acc * 100.0) - 83.06);
    println!("\nGap to 90%:                   {:.2}%", 90.0 - ensemble_acc * 100.0);
    ...
    let results_json = serde_json::json!({
        ...
        "comparison": {
            "previous_best": 0.8306,
            "improvement": ensemble_acc - 0.8306,
        },
        "training_time_minutes": total_elapsed.as_secs() as f64 / 60.0,
    });

    std::fs::write(
        "results/classifier_head_results.json",
        serde_json::to_string_pretty(&results_json)?
    )?;

    println!("\n✓ Results saved to: results/classifier_head_results.json");

    Ok(())
}

fn evaluate(
    model: &mut HybridWithClassifier,
...
fn evaluate_ensemble(
    models: &mut [HybridWithClassifier],
    data: &[(usize, usize, f32)],
    tf_expr_map: &HashMap<usize, Array1<f32>>,
    gene_expr_map: &HashMap<usize, Array1<f32>>,
    expr_dim: usize,
