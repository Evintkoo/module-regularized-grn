/// Dropout ablation: both models with dropout=[0.0, 0.1, 0.2, 0.3], 3 seeds each.
/// Writes results/dropout_ablation.json
use module_regularized_grn::{
    Config,
    models::hybrid_embeddings::HybridEmbeddingModel,
    models::cross_encoder::CrossEncoderModel,
    models::nn::{Dropout, bce_loss_backward},
...
fn main() -> Result<()> {
    ...
    let dropout_rates = [0.0f32, 0.1, 0.2, 0.3];

    let mut all_results: Vec<serde_json::Value> = Vec::new();

    for &drop_rate in &dropout_rates {
        println!("dropout={}", drop_rate);

        // ── Two-Tower with dropout ──
        ...
        for &seed in &seeds {
            ...
            let mut tf_dropout = Dropout::new(drop_rate);
            let mut gene_dropout = Dropout::new(drop_rate);
            ...
            for epoch in 0..epochs {
                ...
                let mut drop_rng = StdRng::seed_from_u64(seed + epoch as u64 + 5000);
                for start in (0..train_shuffled.len()).step_by(batch_size) {
                    ...
                    let preds = model.forward_with_dropout(&tf_idx, &gene_idx, &tf_e, &gene_e,
                        &mut tf_dropout, &mut gene_dropout, true, &mut drop_rng);
                    let grad: Array1<f32> = preds.iter().zip(labels.iter()).map(|(&p, &l)| (p-l)/bsz as f32).collect();
                    model.backward_with_dropout(&grad, &tf_dropout, &gene_dropout);
                    adam_step_tt(&mut model, &mut state, 0.001, 5.0);
                    model.zero_grad();
                }
            }
            // Eval (no dropout)
            ...
            tt_aurocs.push(calculate_auroc(&all_preds, &all_labels));
            tt_accs.push(acc);
        }

        // ── Cross-Encoder with dropout ──
        ...
        for &seed in &seeds {
            ...
            let mut dropout1 = Dropout::new(drop_rate);
            let mut dropout2 = Dropout::new(drop_rate);
            let mut train_shuffled = train_data.clone();
            ...
            for epoch in 0..epochs {
                ...
                for start in (0..train_shuffled.len()).step_by(batch_size) {
                    ...
                    let tf_e = build_expr_batch(&tf_idx, &tf_expr_map, expr_dim);
                    let gene_e = build_expr_batch(&gene_idx, &gene_expr_map, expr_dim);
                    let logits = model.forward_with_dropout(&tf_idx, &gene_idx, &tf_e, &gene_e,
                        &mut dropout1, &mut dropout2, true, &mut drop_rng);
                    let labels_arr = Array1::from(labels);
                    let grad = bce_loss_backward(&logits, &labels_arr);
                    model.backward_with_dropout(&grad, &dropout1, &dropout2);
                    adam_step_ce(&mut model, &mut state, 0.005, 1.0);
                    model.zero_grad();
                }
            }
            // Eval (no dropout)
            ...
            let acc = all_preds.iter().zip(all_labels.iter()).filter(|(&p, &l)| (p >= 0.5) == (l == 1.0)).count() as f32 / all_preds.len() as f32;
            ce_aurocs.push(calculate_auroc(&all_preds, &all_labels));
            ce_accs.push(acc);
        }

        println!("  TT: auroc={:.4}+/-{:.4} acc={:.2}%+/-{:.2}%",
                 mean(&tt_aurocs), std_dev(&tt_aurocs), mean(&tt_accs)*100.0, std_dev(&tt_accs)*100.0);
        println!("  CE: auroc={:.4}+/-{:.4} acc={:.2}%+/-{:.2}%",
                 mean(&ce_aurocs), std_dev(&ce_aurocs), mean(&ce_accs)*100.0, std_dev(&ce_accs)*100.0);

        all_results.push(serde_json::json!({
            "dropout_rate": drop_rate,
            "two_tower": {
                "seed_aurocs": tt_aurocs, "mean_auroc": mean(&tt_aurocs), "std_auroc": std_dev(&tt_aurocs),
                "seed_accs": tt_accs, "mean_acc": mean(&tt_accs), "std_acc": std_dev(&tt_accs),
            },
            "cross_encoder": {
                "seed_aurocs": ce_aurocs, "mean_auroc": mean(&ce_aurocs), "std_auroc": std_dev(&ce_aurocs),
                "seed_accs": ce_accs, "mean_acc": mean(&ce_accs), "std_acc": std_dev(&ce_accs),
            },
        }));
    }
    ...
    std::fs::write("results/dropout_ablation.json", serde_json::to_string_pretty(&output)?)?;
    println!("\n✓ Saved results/dropout_ablation.json");
    Ok(())
}
