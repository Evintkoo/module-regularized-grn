/// Two-tower with focal loss at 5:1 and 10:1 imbalance ratios.
/// Sweeps gamma=[0.5, 1.0, 2.0, 5.0], 3 seeds each.
/// Writes results/two_tower_focal.json
...
fn evaluate_tt(
    ...
    (acc, calculate_auroc(&all_preds, &all_labels), calculate_f1(&all_preds, &all_labels))
}

fn main() -> Result<()> {
    println!("=== Two-Tower Focal Loss Experiment ===\n");
    ...
    let neg_ratios = [5usize, 10];

    let mut all_results: Vec<serde_json::Value> = Vec::new();

    for &neg_ratio in &neg_ratios {
        // Build imbalanced training data
        let neg_extra = builder.sample_negative_examples(pos_train.len() * neg_ratio, base_seed + neg_ratio as u64);
        let mut train_data: Vec<(usize, usize, f32)> = pos_train.clone();
        train_data.extend(neg_extra.iter().map(|&(tf, g)| (tf, g, 0.0f32)));
        let mut shuffle_rng = StdRng::seed_from_u64(base_seed + 1000);
        train_data.shuffle(&mut shuffle_rng);

        for &gamma in &gammas {
            println!("neg_ratio={}:1 gamma={}", neg_ratio, gamma);
            ...
            for &seed in &seeds {
                ...
                for epoch in 0..epochs {
                    ...
                    for start in (0..train_shuffled.len()).step_by(batch_size) {
                        ...
                        // Focal loss gradient: (p - l) * focal_weight / batch_size
                        let grad: Array1<f32> = preds.iter().zip(labels.iter()).map(|(&p, &l)| {
                            let p_t = if l == 1.0 { p } else { 1.0 - p };
                            let focal_weight = (1.0 - p_t).powf(gamma);
                            focal_weight * (p - l) / bsz as f32
                        }).collect();
                ...
                let (acc, auroc, f1) = evaluate_tt(&mut model, &test_data, &tf_expr_map, &gene_expr_map, expr_dim, batch_size);
                seed_accs.push(acc);
                seed_aurocs.push(auroc);
                seed_f1s.push(f1);
            }

            println!("  acc={:.2}%±{:.2}% auroc={:.4}±{:.4} f1={:.4}±{:.4}",
                     mean(&seed_accs)*100.0, std_dev(&seed_accs)*100.0,
                     mean(&seed_aurocs), std_dev(&seed_aurocs),
                     mean(&seed_f1s), std_dev(&seed_f1s));
    ...
    let output = serde_json::json!({
        "experiment": "two_tower_focal_loss",
        "gammas": gammas,
    ...
    std::fs::write("results/two_tower_focal.json", serde_json::to_string_pretty(&output)?)?;
    println!("\n✓ Saved results/two_tower_focal.json");
    Ok(())
}
