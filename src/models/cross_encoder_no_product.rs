/// CrossEncoderNoProduct: ablation of CrossEncoderModel with element-wise product term removed.
/// Input = [tf_emb | gene_emb | tf_expr | gene_expr] (2*embed_dim + 2*expr_dim).
/// Used for Reviewer 2 M1 ablation.
...
pub struct CrossEncoderNoProductModel {
    ...
    pub fc1: LinearLayer,  // (2*embed+2*expr) → hidden, ReLU
...
impl CrossEncoderNoProductModel {
    pub fn new(
        num_tfs: usize,
        num_genes: usize,
        embed_dim: usize,
        ...
        let input_dim  = 2 * embed_dim + 2 * expr_dim;
    ...
    pub fn forward(
        ...
        // Build input [batch, 2*embed+2*expr] — no interaction term
    ...
    /// grad_input layout: [g_tf_emb (0..e) | g_gene_emb (e..2e) | g_tf_expr (2e..2e+x) | g_gene_expr (2e+x..2e+2x)]
    pub fn backward(&mut self, grad_output: &Array2<f32>) {
        ...
        // grad_input: [batch, 2*embed_dim + 2*expr_dim]

        let e = self.embed_dim;

        // Split gradients from concatenated input (no interaction term to backprop)
...
mod tests {
    ...
    fn test_forward_output_shape() {
        ...
        let mut model = CrossEncoderNoProductModel::new(10, 20, 4, 3, 8, 42);
    ...
    fn test_loss_decreases_after_sgd_step() {
        let mut model = CrossEncoderNoProductModel::new(10, 20, 4, 3, 8, 99);
        let tf_idx    = vec![0usize, 1, 2, 3];
        let gene_idx  = vec![5usize, 6, 7, 8];
        let tf_expr   = Array2::<f32>::zeros((4, 3));
        let gene_expr = Array2::<f32>::zeros((4, 3));
